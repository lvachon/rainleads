...
<html>
	<head>
		...
		<script src="<?=$HOME_URL?>js/edit-in-place.js"></script>
		...
		$(function() { 
			...
			$('body').editables( { 
		      ...
		      onFreeze: function(){
			    $.post("save.php",{"editfield":"MD"+this.id,"editval":this.value},function(data){
				});
		      }
		    });
		});
...
$FUCKINGDEFAULTS = array("service1"=>"Fence Whitewashing","service2"=>"Widget Building","service3"=>"Imports / Exports","service4"=>"Fence Whitewashing","service5"=>"Widget Building","service6"=>"Imports / Exports","address"=>"123 Main St.","address2"=>"Suite 321","city_state"=>"City,ST 01234","tel"=>"555-555-1212","fax"=>"555-555-1210","email"=>"name@domain.com","url"=>"www.domain.com","hours"=>"Mon-Fri, 9-5","company"=>$account->title,"desc"=>"Lorem ipsum dolor sit amet, consectetur adipiscing elit. Vivamus blandit elementum dictum. In hac habitasse platea dictumst. Aliquam erat volutpat. Nam consectetur mollis ornare. Pellentesque sodales hendrerit est, imperdiet tempor lorem suscipit eget.");
$ACCOUNTDEFAULTS = array("city_state"=>$account->data['city'].", ".$account->data['state'],"address"=>$account->data['address'],"address2"=>$account->data['address2'],"tel"=>$account->data['phone']);//Of course the field names aren't the same, that would make too much fucking sense.   FUCKING FUCK! THEY AREN"T EVEN THE SAME STRUCTURE, FUCK YOU!
function Z($field){
	...
	return $FUCKINGDEFAULTS[$field];
	
}?>
$(document).ready(function(){
	<?php 
	foreach($FUCKINGDEFAULTS as $FUCK=>$COCK){
		?>$("#<?=$FUCK?>").val("<?=htmlentities(Z($FUCK));?>");
		  $("label[data-field='<?=$FUCK?>']").html("<?=htmlentities(Z($FUCK));?>");
		<?php
	} ?>
});
</script>
	...
	<body>
		<div id="box">
			<div class="left" id="side">
				...
				<div id="address">
					<span class="editable">
						<label data-type="editable" data-for="input#address" data-field="address">123 Main St.</label>
						<input class="eip_input" id="address" />
					</span>
					<span class="editable">
			...
			<div class="left" id="main">
				<h1 class="editable">
						<label data-type="editable" data-for="input#company" data-field="company"><?=$account->title;?></label>
