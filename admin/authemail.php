...
if(!$plan){errorMsg("No plan found with that name: {$_POST['plan']}'");die();}
$xml .= "	<subscription>\n";
...
$xml .= "				<expirationDate>20{$_POST['expyr']}-{$_POST['expmo']}</expirationDate>\n";
$xml .= "				<cardCode>{$_POST['ccv']}</cardCode>\n";
$xml .= "			</creditCard>\n";
$xml .= "		</payment>\n";
$xml .= "		<customer>\n";
...
if ($response)
{
	...
	*/
	...
	fclose($fp);
	$con = conDB();
	mysql_query("UPDATE accounts set sub_id='".mysql_escape_string($subscriptionId)."', plantype='".mysql_escape_string($plan['name'])."' where id={$account->id} LIMIT 1",$con);
